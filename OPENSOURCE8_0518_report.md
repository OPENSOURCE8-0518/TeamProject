...
- 문서 갱신일 : 2022/12/04
- 문서 기여자 : 김지원(조장), 김지은, 김지현, 김지효, 정구현, 정세얀, 정영한
...
## 개요
 ...
 이 시스템 설계서는 '캠퍼스 단위 개인화 커뮤니티'의 구현을 위해 작성되었다.

### 서비스의 목적

 이 서비스는 기존 캠퍼스 단위 커뮤니티에서 검색 기능과 개인화 서비스를 보완한 커뮤니티 서비스를 제공하는 것을 목적으로 한다.

### 추진 배경

 한국 핀테크 연구회가 약 1천명의 대학생을 대상으로 진행한 조사에 따르면, 캠퍼스 커뮤니티인 에브리타임이 꼭 필요한 서비스라고 대답한 비율이 84%를 차지한다. 이로 미루어 볼 때, 캠퍼스 단위 커뮤니티는 대다수 대학생에게 매우 중요한 서비스로 인식됨을 확인할 수 있었다. 그러나, 현재 가장 활발하게 사용되는 커뮤니티인 에브리타임에도 몇가지 단점이 존재한다.
 첫 번째로, 검색어의 유동성 부족이다. 에브리타임의 많은 유저들은 교수님의 성함 혹은 강의명을 작성할 때 직접적인 언급을 피하기 위해 초성 형태로 적는 경우가 많은데, 이 경우 기존 검색어와 관련된 내용임에도 검색하기 힘든 경우가 존재한다.
 두 번째로, 인스타그램, 유튜브를 비롯한 많은 서비스처럼 자주 검색하는(열람하는) 게시물을 알아서 추천해주는 개인화 기능이 존재하지 않는다. 특정 학과의 학생의 경우, 자신의 학과와 관련된 게시물을 더 높은 빈도로 보기를 원할 가능성이 높으나, 이런 개인화 기능을 지원하지 않기 때문에 매번 모든 게시글을 보아야 한다.
...
#### 신뢰도 문제(Reliability)
- 특정 강의를 수강하지 않은 사람이 그 강의에 대해 글을 쓴다면?

 직장인을 대상으로 하는 커뮤니티인 블라인드(Blind)의 경우, 사용자는 익명이지만 사용자 옆에 어떤 회사에 제직중인지 확인이 가능하다. 해당 기능을 밴치마킹해 특정 트랙/학과를 다니고 있음을 인증해 이름 옆에 뜨도록 하면 해당 강의와 완전히 상관없는 인물이 글을 작성하는 경우를 막을 수 있어 최소한의 신뢰성을 보장할 수 있을 것으로 보인다.
 또한, 게시글의 '좋아요' 기능을 통해 부정확한 글을 일부 정제해 확인할 수 있을 것으로 사료된다.

- Bert에 의해 학습될 데이터의 신뢰도는?

  트랙 정보의 경우, 대학교 홈페이지로부터 직접 가져오므로 신뢰성에 문제가 없으나, 트랙 구조조정이 발생한 경우 갱신되는데 시간이 필요하므로 이 경우 직접 데이터를 입력해 수정할 수 있다. 트랙 조정의 경우 아무리 빠르더라도 1년의 시간을 소요하므로, 자주 갱신할 필요는 없다.
  또한 에브리타임으로부터 가져오는 데이터의 경우, 일반적으로 글 제목을 스크랩해 학습할 예정으로, 사용자들이 자주 사용하는 글 제목을 학습해 이러한 제목도 검색할 수 있도록 하는 기능이므로 신뢰도와는 관계가 없다.

##### 참고
지식검색커뮤니티 정보의 신뢰성에 관한 연구 동향 분석(2012, 정보관리학회지, 김수정, p.17)
...
## 대학생 커뮤니티 '에브리타임'
- 특정 대학에 재학중인 학생들끼리만 이용할 수 있는 커뮤니티.
- 다양한 주제의 게시판이 존재하고 그 중 자유게시판에는 유익한 정보글부터 그냥 잡담까지 아주 다양한 주제의 글이 올라옴.
- 일반적인 검색어로 검색하는 경우 초성으로 된 관련 글을 찾을 수 없음.
- 개인화 기능 부재.

## 캠퍼스 픽
- 에브리타임의 파생서비스.
- 모든 대학생들을 대상으로 함.
- 개인이 쉽게 개설하는 소규모 커뮤니티가 모여 있는 형태.
- 7만명 이상이 구독중인 커뮤니티도 하루에 글이 10개도 올라오지 않을 만큼 글이 올라오는 빈도가 저조함.
...
### 설명
 ...
 아파치 소프트웨어 재단에서 관리하는 오픈소스 HTTP 웹 서버 소프트웨어이다.
 웹서버 게이트웨이를 통해 웹 애플리케이션과 연결해 사용할 수 있다.

### 특징

 다양한 모듈을 통해 수많은 기능을 추가하고 관리할 수 있다. 일반적으로 PHP와 MySQL을 설치해 사용한다.

### 사용법

 리눅스 서버에 설치해 사용할 수 있다.
...
```
이후 conf 파일을 수정해 서버명, 프로젝트 경로, 라이브러리 경로등을 입력해 연동한다.

### 라이선스

Apache License 2.0
**아파치 라이센스의 경우 원저작물을 어떠한 방식으로든 사용할 수 있다. 2차 저작물(파생결과물/Derivative Works)에 대해서는 소스를 공개하지 않아도 되지만, 이 경우에는 수정된 부분을 표시하여야 한다.**

### 참고

Apache HTTP Server Project 공식 홈페이지 : https://httpd.apache.org/
Wikipedia 아파치 HTTP 서버 : https://ko.wikipedia.org/wiki/%EC%95%84%ED%8C%8C%EC%B9%98_HTTP_%EC%84%9C%EB%B2%84
리눅스에 APM(Apache + PHP + MySQL) 설치 및 연동 : https://hoing.io/archives/183
Django, Apache 연동하기 : https://dontdiethere.tistory.com/29
...
위키피디아 - Django (웹 프레임워크) 문서 : https://ko.wikipedia.org/wiki/%EC%9E%A5%EA%B3%A0_(%EC%9B%B9_%ED%94%84%EB%A0%88%EC%9E%84%EC%9B%8C%ED%81%AC)#cite_note-14

깃허브 Django 주소 : https://github.com/django/django
...
**Community Edition을 사용한다.**

### 설명

오픈 소스 RDBMS(관계형 데이터베이스)

### 특징

 Key - Value 형식으로 데이터들을 테이블에 저장한 뒤 테이블들을 모아 스키마를 형성한다.

 유닉스, 리눅스, 윈도우 등의 OS에서 사용가능하며, 빠르고, 유연한 것이 장점이다.

 언어로 SQL을 사용한다.

### 사용

DB-connector와 jdbc를 통해 Elastic Search logstash와 연결해 사용할 수 있다.
```
$ sudo apt-get install mysql-connector-java

$ bin/logstash-plugin install logstash-integration-jdb
```
이후 conf 파일을 수정하는 것으로 MySQL에 저장된 데이터와 Elastic Search간 쿼리 싱크를 맞추면 사용할 수 있다.
...
MySQL 데이터를 ElasticSearch로 검색하기 : https://velog.io/@broccoliindb/mysql-%EB%8D%B0%EC%9D%B4%ED%84%B0%EB%A5%BC-elasticsearch%EB%A1%9C-%EA%B2%80%EC%83%89%ED%95%98%EA%B8%B0
...
MySQL과 Elastic Search를 연결해 데이터를 수집할 수 있도록 한다.
동일한 Apache License 2.0을 사용한다.
...
**Elastic Search의 경우 2021년부로 라이선싱 관련으로 변경된 부분이 있다. 이에 Apache License 2.0을 적용하기 위해 해당 서비스에서 사용하는 Elastic Search의 버전은 7.10버전임을 명시한다.**
...
엘라스틱 서치 라이센스 변경 안내 : https://www.elastic.co/kr/pricing/faq/licensing
...
깃허브 KoNLPy 패키지 : https://github.com/konlpy/konlpy
...
깃허브 Okt 태거 클래스 : https://github.com/open-korean-text/open-korean-text
...
Elastic Search LTR Github : https://github.com/o19s/elasticsearch-learning-to-rank
...
IGListKit 구조 : https://leejigun.github.io/IGListKit
...
접속 과정
1. 유저가 아파치 웹서버에 웹페이지 요청.
2. 아파치가 장고에 웹페이지 요청.
3. 장고가 구축된 웹페이지 전달
4. 아파치가 전달받은 웹페이지 다시 유저에게 전달.
5. 커뮤니티에 접속 완료.

검색 과정
1. 유저가 웹서버에 검색 패킷 전달.
2. 웹페이지(장고)가 전달받은 검색어와 사전에 등록되어있던 선호 검색어를 코엔엘파이에 전달.
3. 코엔엘파이가 검색어를 추출해 리스트 형태로 버트에게 전달.
4. 버트는 대학 사이트 혹은 에브리타임에서 미리 scrapy를 통해 크롤링된 데이터로 학습됨.
5. 버트가 검색어와 학습 정보를 대조해 관련성이 높은 검색어를 Elastic searc와 individualizer에 전달.
6. 엘라스틱서치에 넘어온 검색어를 바탕으로 내부에 저장된 글 목록으로부터 검색함.
7. 검색 결과를 LTR을 통해 우선순위로 정렬.
8. IGListkit을 통해 좋아요 반영 및 시각화함.
10. 장고에 넘어온 결과를 웹페이지에 업데이트.
11. 아파치가 갱신된 웹페이지를 유저에게 전달.

글 작성 과정
1. 유저가 사이트에 글 작성
2. 아파치가 데이터 장고에 전달
3. 장고가 전달받은 데이터(글)을 서버 데이터배이스에 전달해 저장.
...
**소스 코드 공개의 의무가 없고, 2차 라이선스와 변형물의 특허 출원 가능. 라이선스 적용 시 아파치 재단의 이름과 라이선스의 내용을 명시해야 하며, 아파치 라이선스 2.0이 적용된 소스코드를 수정했을 경우 외부에 그 사실을 밝혀야 함**

**서비스가 배포되었을 때 라이선스에 따른 의무사항이 발생하나, 웹 서비스는 프로그램 소스를 직접 배포하는 것이 아닌 네트워크를 통해 서비스만을 제공하므로 의무사항이 발생하지 않음 (AGPL-3.0 라이선스 제외)**
